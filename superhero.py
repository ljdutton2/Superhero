...
class Arena:
    ...
    def build_team_one(self):
       ...
       team_1_heroes = int(input(f"How many heroes are on {}? "))
       for _ in team_1_heroes:
           hero = self.create_hero()
           first_team.add_hero(hero)
      	return first_team.append(self.team_one)

    def build_team_two(self):
		team_2_name = input("Choose a name for Team 2")
		second_team = Team(team_2_name)
		team_2_heroes = int(input(f"How many heroes are on {}? ""))
		for _ in team_2_heroes:
			hero = self.create_hero()
			second_team.add_hero(hero)
		return second_team.append(self.team_two)
